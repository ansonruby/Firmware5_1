...
# Enviar_QR(IP,T_actual,ID,QR):
# Pedir_Usuarios_Activos(IP,T_actual,ID):
# Enviar_Usuarios(IP,T_actual,ID,Usuarios):
# Ping (IP):
# Veri_Firmware(IP, T_actual, ID, vercion_Actual_Firmware):
